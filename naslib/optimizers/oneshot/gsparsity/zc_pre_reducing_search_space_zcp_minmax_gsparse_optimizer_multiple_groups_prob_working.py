...
class ZCP_GSparseOptimizer(MetaOptimizer):
    ...
    def adapt_search_space(
        ...
        **kwargs,
        ...
        # ------------------------------------------------------------------
        # 0) PRE-REDUCTION PHASE (act on raw op lists before MixedOp/alphas)
        # ------------------------------------------------------------------
        # Add per-edge beta flags (private to each edge's op list)
        add_betas_to_edges(graph, scope=scope)
        logger.info("Added beta parameters to search space edges.")

        # Option A: resume from checkpoint and re-apply removals
        ...
        # Option B: use pre-computed zc scores to prune
            ...
            if os.path.exists(duration_path):
                ...
                jacov_scores = np.array([op_to_score[a]["jacov"] for a in arch_tuples])
                synflow_scores = np.array(
                    [op_to_score[a]["synflow"] for a in arch_tuples]
                )
                param_scores = np.array([op_to_score[a]["params"] for a in arch_tuples])

...
#SBATCH --job-name=wide_hpo_queried_val_acc_{optimizer}_{dataset}_{seed}{job_suffix}
#SBATCH --output=naslib/optimizers/oneshot/gsparsity/results_wide_hpo_queried_val_acc/slurm/wide_hpo_{optimizer}_{dataset}_{seed}{zcp_out_suffix}_%j.out
...
def generate_slurm_script(optimizer, dataset, seed, zcp_method):
    ...
    base_cmd = [
        "python -u naslib/optimizers/oneshot/gsparsity/high_budget_wide_hpo_search_space_configurator_queried_val_acc_based.py",
        f"--optimizer {optimizer}",
        "--search_space nasbench201",
        f"--dataset {dataset}",
        f"--seed {seed}",
        "--resume True",
        "--out_dir naslib/optimizers/oneshot/gsparsity/results_wide_hpo_queried_val_acc",
    ]
    if zcp_method:
...
def main():
    ...
    for optimizer in optimizers:
        for dataset in datasets:
            output_dir = f"naslib/optimizers/oneshot/gsparsity/submission_scripts/wide_hpo_queried_val_acc/{optimizer}"

...
from .pruner import DEHBPruner, _clean_trials
...
class DEHBSampler(BaseSampler):
    ...
    def sample_relative(
        ...
        # Get all trials for history, including failed and running ones for cleaning.
        all_trials = study._get_trials(
            deepcopy=False,
            states=(
                TrialState.COMPLETE,
                TrialState.PRUNED,
        ...
        logging.info(f"All Trials in relative Sampling: {len(all_trials)}")

        # Clean the history to get a reliable set of trials for parent selection.
        cleaned_trials = _clean_trials(study, all_trials, trial)
        logging.info(
            f"Trials in relative Sampling after cleaning: {len(cleaned_trials)}"
        )

        # Use only successfully completed or pruned trials for the logic below.
        # This is crucial for DEHB's core algorithm.
        completed_trials = [
            t
            for t in cleaned_trials
            if t.state in [TrialState.COMPLETE, TrialState.PRUNED]
        ]

        if len(completed_trials) <= self._get_n_trials_in_first_dehb_iteration(study):
            return {}

        if len(completed_trials) < 3:
            # Not enough completed trials in history to perform DE, fallback to random.
            return {}
        ...
        assert iteration_id > 0, (
            f"DEHB internal error: iteration_id must be > 0. "
            f"Calculated for trial {trial.number}."
        )
        logging.info("Using DE part of DEHB sampler.")
        ...
        if len(subpopulation) < 3:
            subpopulation.extend(completed_trials)
    ...
    def sample_independent(
        ...
        ret = self._random_sampler.sample_independent(

...
# Correct source and target folders
original_data_folder = "naslib/optimizers/oneshot/gsparsity/submission_scripts/nasbench201_zc_scoring_timefactor"
new_folder = os.path.join(
    original_data_folder, "nasbench201_zc_scoring_timefactor_dur0"
)

# Build dur0 once: copy all JSONs; zero only duration JSONs
if not os.path.exists(new_folder):
    os.makedirs(new_folder, exist_ok=True)
    for filename in os.listdir(original_data_folder):
        if not filename.endswith(".json"):
            continue
        src = os.path.join(original_data_folder, filename)
        dst = os.path.join(new_folder, filename)
        try:
            if filename.startswith("arch_scores_duration_"):
                # Zero duration
                with open(dst, "w") as f:
                    json.dump({"duration": 0.0}, f)
            else:
                # Copy scores unchanged
                shutil.copy2(src, dst)
        except Exception as e:
            print(f"[dur0] Failed to process {filename}: {e}")
    ...
    if optimizer_type == "gsparsity":
        ...
        config = {
            "search": {
                ...
                "batch_size": trial.suggest_categorical("batch_size", [32, 64, 128]),
                "train_portion": trial.suggest_float("train_portion", 0.8, 0.99),
                "pre_computed_zc_scores": os.path.join(new_folder, "arch_scores"),
            },
        }
...
def update_config(
    ...
    config.search.hpo_objective = "qva"
...
def run_optimizer(optimizer_type, search_space_type, dataset, config, seed, trial):
    ...
    if search_trajectory and search_trajectory.queried_val_acc:
        final_val_acc = search_trajectory.queried_val_acc[-1]
